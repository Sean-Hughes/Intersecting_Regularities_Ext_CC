---
Title: "Meta analyses of IR effects"
Author: "Ian Hussey"
---
...
```{r}
# convert SPSS to csv
input_data_exp_1_IR_Effects <- 
  read.spss("Meta_Analyses_IR_Effects.sav", to.data.frame = TRUE)

# tidy data
tidied_data_exp_1_IR_Effects <- input_data_exp_1_IR_Effects %>%
  mutate(Training_Condition = str_remove_all(Training_Condition, " "))

write_csv2(tidied_data_exp_1_IR_Effects, "IR_Effect_Sizes.csv")
...
data_IR_effects <- read.csv2("IR_Effect_Sizes.csv") %>%
...
data_IR_effects_counterconditioning <- data_IR_effects %>%
  filter(Training_Condition == "Counterconditioning")

# fit a default REML meta analyis model
fit_cc <- rma.mv(yi = yi,
                 V = sei^2,
                 data = data_IR_effects_counterconditioning,
                 slab = paste(Experiment, DV),
                 random = ~ DV | Experiment) # multiple DVs per participant, nested by experiment
...
data_IR_effects_extinction <- data_IR_effects %>%
  filter(Training_Condition == "Extinction")

# fit a default REML meta analyis model
fit_ex <- rma.mv(yi = yi,
                 V = sei^2,
                 data = data_IR_effects_extinction,
                 slab = paste(Experiment, DV),
                 random = ~ DV | Experiment) # multiple DVs per participant, nested by experiment

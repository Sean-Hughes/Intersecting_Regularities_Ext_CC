...
temp_study_1 <- data_reshaped_intentions_acquisition_ir_sensitivity %>%
  filter(experiment == 1) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_2 <- data_reshaped_intentions_acquisition_ir_sensitivity %>%
  filter(experiment == 2) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_3 <- data_reshaped_intentions_acquisition_ir_sensitivity %>%
  filter(experiment == 3) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_4 <- data_reshaped_intentions_acquisition_ir_sensitivity %>%
  filter(experiment == 4) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_5 <- data_reshaped_intentions_acquisition_ir_sensitivity %>%
  filter(experiment == 5) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_6 <- data_reshaped_intentions_acquisition_ir_sensitivity %>%
  filter(experiment == 6) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_7 <- data_reshaped_intentions_acquisition_ir_sensitivity %>%
  filter(experiment == 7) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)
...
fit_basic_effect_intentions_ir_sensitivity <- data_reshaped_intentions_acquisition_ir_sensitivity %>%
  pivot_wider(names_from = stimulus_identity_condition,
              values_from = c(brand_A, brand_B)) %>%
  escalc(measure = "OR",
         ai = brand_A_Outcome_1_Target_1_Good_Outcome_2_Target_2_Bad,
...
temp_study_1 <- data_reshaped_intentions_moderation_by_extinction_ir_sensitivity %>%
  filter(experiment == 1) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_2 <- data_reshaped_intentions_moderation_by_extinction_ir_sensitivity %>%
  filter(experiment == 2) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_3 <- data_reshaped_intentions_moderation_by_extinction_ir_sensitivity %>%
  filter(experiment == 3) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_4 <- data_reshaped_intentions_moderation_by_extinction_ir_sensitivity %>%
  filter(experiment == 4) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_7 <- data_reshaped_intentions_moderation_by_extinction_ir_sensitivity %>%
  filter(experiment == 7) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

results_moderation_by_extinction_intentions_ir_sensitivity <-
  bind_rows(
    as.data.frame(temp_study_1$tab) %>%
      drop_na() %>%
      select(OR = oddsratio, OR_ci_lower = lower, OR_ci_upper = upper, p = p.value) %>%
...
fit_moderation_extinction_ir_sensitivity <- data_reshaped_intentions_moderation_by_extinction_ir_sensitivity %>%
  pivot_wider(names_from = experiment_condition,
              values_from = c(brand_A, brand_B)) %>%
  escalc(measure = "OR",
         ai = brand_A_acquisition_only,
...
temp_study_5 <- data_reshaped_intentions_moderation_by_counterconditioning_ir_sensitivity %>%
  filter(experiment == 5) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_6 <- data_reshaped_intentions_moderation_by_counterconditioning_ir_sensitivity %>%
  filter(experiment == 6) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

temp_study_7 <- data_reshaped_intentions_moderation_by_counterconditioning_ir_sensitivity %>%
  filter(experiment == 7) %>%
  select(brand_A, brand_B) %>%
  as.matrix() %>%
  epitools::epitab(x = .)

results_moderation_by_counterconditioning_intentions_ir_sensitivity <-
  bind_rows(
    as.data.frame(temp_study_5$tab) %>%
      drop_na() %>%
      select(OR = oddsratio, OR_ci_lower = lower, OR_ci_upper = upper, p = p.value) %>%
...
fit_moderation_counterconditioning_ir_sensitivity <- data_reshaped_intentions_moderation_by_counterconditioning_ir_sensitivity %>%
  pivot_wider(names_from = experiment_condition,
              values_from = c(brand_A, brand_B)) %>%
  escalc(measure = "OR",
         ai = brand_A_acquisition_only,
...
- move "  ## move this to data processing??" chunk to data processing
- study 7 data processing 
  - duplicate unique_ids in processed data?
  <!-- x <- read_csv("../data/Study 7/processed/data_processed.csv") %>% -->
  <!--   arrange(unique_id) %>% -->
  <!--   filter(unique_id == lag(unique_id)) -->
  - why do the Ns for the extinction and counterconditioning training and testing processed data equal the acquisition phase, when only one third of people should have done CC and extinction (each)?
...
- Moderation of OEC by extinction (self-reported evaluations) seems to go beyond limits of forest plot. when data processing is finalized, check that this and all plots' limits are appropriate and standardized.

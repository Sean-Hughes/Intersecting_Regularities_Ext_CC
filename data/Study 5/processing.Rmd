...
- exploratory_questions.xlsx currently not used
- reverse scoring done for self reported ratings, but what about other self-reports and iats?
- ensure reverse scoring is congruent between outcome measures
...
- assess whether all raw data files are used by this processing script or not. if not, why? this may also differ for other studies.

- why is there only one extinction training file? accuracy by block doesn't seem to be working
...
intersecting_regularities_left.xlsx & 
intersecting_regularities_right.xlsx: 
- Outcome 1 and Target 1 are Good/ Outcome 2 and Target 2 are Bad

intersecting_regularities_left_rev.xlsx & 
intersecting_regularities_right_rev.xlsx: 
...
data_extinction_condition <- 
  ...
  mutate(extinction_condition = "acquisition and counterconditioning") %>%
...
data_training_and_testing_completeness_temp <- data_training_and_testing_input %>%
  ...
  mutate(complete_training_testing_data = 
           case_when((phase == "counterconditioning_training" & n == 80) |
...
temp <- 
  data_processed %>%
  filter(completeness_counterconditioning_testing == "complete" & 
           ...
           passed_performance_criterion_counterconditioning_testing == TRUE &

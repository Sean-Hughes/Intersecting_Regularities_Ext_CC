...
# stimulus identity using counterconditioning_training file names
...
data_experimental_condition <- 
  bind_rows(
    read_xlsx("raw/counterconditioning1a_intersecting_regularities_left.xlsx") %>% 
      mutate(response = as.character(response)),
    read_xlsx("raw/counterconditioning1a_intersecting_regularities_left_rev.xlsx") %>% 
      mutate(response = as.character(response)),
    read_xlsx("raw/counterconditioning1a_intersecting_regularities_right.xlsx") %>% 
      mutate(response = as.character(response)),
    read_xlsx("raw/counterconditioning1a_intersecting_regularities_right_rev.xlsx") %>% 
      mutate(response = as.character(response)),
  ) %>%
  mutate(experimental_condition = "acquisition and counterconditioning") %>%
  select(subject, experimental_condition) %>%
  distinct(subject, .keep_all = TRUE)
...
data_conditions <- data_stimulus_identity_condition %>%
  full_join(data_experimental_condition, by = "subject") %>%
  full_join(data_task_order, by = "subject") %>%
  mutate(experiment = current_experiment_number,
         experimental_condition = ifelse(is.na(experimental_condition), "acquisition only", experimental_condition))

rm(data_stimulus_identity_condition, data_experimental_condition, data_task_order)
...
data_training_and_testing_completeness_temp <- data_training_and_testing_input %>%
  ...
  mutate(complete_training_testing_data = 
           case_when((phase == "acquisition_training" & n == 80) |
                       ...
                       (phase == "counterconditioning_training" & n == 80) ~ "complete", 
                     (phase == "acquisition_training" & n > 80) |
                       (phase == "acquisition_testing" & n > 8) |
                       (phase == "counterconditioning_training" & n > 80) ~ "excess", 
                     (phase == "acquisition_training" & n < 80) |
                       (phase == "acquisition_testing" & n < 8) |
                       (phase == "counterconditioning_training" & n < 80) ~ "partial")) %>%
  ungroup() %>%
  select(-n) 
...
data_processed <- data_conditions %>%
  ...
  select(experiment, 
         ...
         task_order, 
         completeness_counterconditioning_training, completeness_counterconditioning_training, 
         mean_correct_counterconditioning_training, mean_correct_counterconditioning_training, 
         passed_performance_criterion_counterconditioning_training, passed_performance_criterion_counterconditioning_training, 
...
temp <- 
  data_processed %>%
  filter(completeness_counterconditioning_training == "complete" & 
           completeness_counterconditioning_training == "complete" & 
           complete_iat_data == "complete" &
           passed_performance_criterion_counterconditioning_training == TRUE &
           passed_performance_criterion_counterconditioning_training == TRUE &
  ...
  select(stimulus_identity_condition, 
         OEC_effect_native, 
...
temp %>%
  group_by(stimulus_identity_condition) %>%
...
temp %>%
  gather(key, value, c(OEC_effect_native, 
                       ...
                       #IAT_D2_native_blockreversed)) %>%
                       #IAT_D2_reverse_scored_blockreversed
  ggplot(aes(value, fill = stimulus_identity_condition)) +

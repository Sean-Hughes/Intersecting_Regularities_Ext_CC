...
# Participants in Experiment 7 were exposed to a methodology that was more complex that the previous experiments. To compensate for this we provided them with an extra block of trials (Block 5) if they failed to reach a certain criteria on Block 4. This means that the amount of complete data for those who needed an extra block is different from that of those who did not need said block.

# Inspecting the data indicated that a small number of participants received more than 5 blocks. These outliers have been trimmed here.
  
  filter(block_number %in% c(1, 2, 3, 4, 5))
...
# label participants as having complete, excess, or partial data
...
data_training_and_testing_completeness_temp <- data_training_and_testing_input %>%
  ...
  mutate(complete_training_testing_data = 
           case_when((phase == "acquisition_training" & n == 80 | n == 100) |  
                       (phase == "acquisition_testing" & n == 8) |
                       (phase == "counterconditioning_training" & n == 80  | n == 100) |  
                       (phase == "counterconditioning_testing" & n == 8) |  
                       (phase == "extinction_training" & n == 80  | n == 100) |  
                       (phase == "extinction_testing" & n == 8) ~ "complete",
                       (phase == "acquisition_training" & n > 100) |  
                       (phase == "acquisition_testing" & n > 8) |
                       (phase == "counterconditioning_training" & n > 100) |  
                       (phase == "counterconditioning_testing" & n > 8) |  
                       (phase == "extinction_training" & n > 100) |  
...
data_intentions <- 
  ...
  distinct(subject, behavioral_intentions_OEC, behavioral_intentions_IR)
...
data_iat_block_order <- data_iat_input %>%
  ...
  distinct(subject, iat_block_order)
...
data_equivalence <- 
  ...
  summarize(n = n(), # check completeness
            passed_equivalence = ifelse(mean(correct) > .75, TRUE, FALSE)) %>%
  ungroup() %>%
  mutate(complete_equivalence_data = ifelse(n == 24, TRUE, FALSE)) %>%
  select(subject, complete_equivalence_data, passed_equivalence)

data_equivalence %>%
  count(complete_equivalence_data, passed_equivalence) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
